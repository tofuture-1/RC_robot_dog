 ...
 * @param __Angle_Max 最大位置, 与上位机控制幅度K_MAX保持一致, 传统模式有效
 * @param __Omega_Max 最大速度, 与上位机控制幅度V_MAX保持一致, 传统模式有效
 * @param __Torque_Max 最大扭矩, 与上位机控制幅度T_MAX保持一致, 传统模式有效
 */
void Class_Motor_DM_Normal::Init(CAN_HandleTypeDef *hcan, uint16_t __CAN_Rx_ID, uint16_t __CAN_Tx_ID, Enum_Motor_DM_Control_Method __Motor_DM_Control_Method, float __Angle_Max, float __Omega_Max, float __Torque_Max, float __Current_Max)
{
    ...
    {
        CAN_Manage_Object = nullptr;  // CAN3不存在于STM32F4
    }
 ...
 * @param __DJI_Motor_Driver_Version 大疆驱动版本, 当且仅当当前被分配电机为6020, 且是电流驱动新版本时选023, 否则都是default
 ...
 *
 */
void Class_Motor_DM_Normal::RTOS_100ms_Alive_Callback()
{
    // 判断该时间段内是否接收过电机数据
    if (Flag == Pre_Flag)
    {
 ...
 *
 */
void Class_Motor_DM_Normal::RTOS_Send_Callback()
{
 ...
 *
 */
void Class_Motor_DM_Normal::Dog_Send_Callback()
{
...
void Class_Motor_DM_Normal::Data_Process()
{
 // 数据处理过程
    ...
    Rx_Data.Control_Status = static_cast<Enum_Motor_DM_Control_Status_Normal>(tmp_buffer->Control_Status_Enum);
		...
		if (Rx_Data.First_Frame)
    {
        Rx_Data.Pre_Encoder = tmp_encoder;
        Rx_Data.Total_Round = 0;
        Rx_Data.First_Frame = false; // 之后不再进入此分支
    }
    ...
    Rx_Data.Total_Encoder = (Rx_Data.Total_Round << 16) + tmp_encoder;

    // 计算电机本身信息
    Rx_Data.Now_Angle = ((float)(Rx_Data.Total_Encoder) / (float)(1 << 16))* 2.0f * Angle_Max;
...
void Class_Motor_DM_Normal::Output()
{
    ...
    case (Motor_DM_Control_Method_NORMAL_MIT):
    {
        ...
        tmp_angle = Math_Float_To_Int(Control_Angle, -Angle_Max, Angle_Max, 0, (1 << 16) - 1);
 ...
 *
 */
void Class_Motor_DM_1_To_4::RTOS_100ms_Alive_Callback()
{
    // 判断该时间段内是否接收过电机数据
    if (Flag == Pre_Flag)
    {
 ...
 *
 */
void Class_Motor_DM_1_To_4::RTOS_Calculate_Callback()
{
    PID_Calculate();
...
void Class_Motor_DM_1_To_4::Data_Process()
{
    ...
    Rx_Data.Total_Encoder = (int32_t)(Rx_Data.Total_Round * Encoder_Num_Per_Round + tmp_encoder) + Encoder_Offset;
